...
import custom_kernels_sal as ck
...
def run_multiple_simulations(year, month, day):
    ...
    output_file_name = "SATELLITE_OUT/SIM1_EU_{year_start:04d}_{month_start:02d}_{day_start:02d}.zarr"
    ...
    # release_lon1 = np.linspace(-57, -49.08333333333333333, 8*12)    # 12 points in longitude
    # release_lat1 = np.linspace(7, 16, 9*12+1)                       # 12 points in latitude

    # release_lon2 = np.linspace(-49, -41.08333333333333333, 8*12)
    # release_lat2 = np.linspace(1, 16, 15*12+1)

    # release_lon3 = np.linspace(-41, -21, 20*12+1)
    # release_lat3 = np.linspace(-1, 16, 17*12+1)
    ...
    # GRIDMASK200 = GRIDMASK200.rename({'x': 'lon', 'y': 'lat'}) # Rename coordinates for compatibility

    # grid_lon, grid_lat = gi.grid_creation(
    ...
    image_name_path = '/storage/shared/oceanparcels/output_data/data_Elena/SATELLITE/C20241772024183.1KM.C_ATLANTIC.7DAY.L3D.FA_UNET_DENSITY.png'
    sarg_lon_grid, sarg_lat_grid, amount = gi.sarg_grid_from_sat(image_name_path, 22.0, 0.0, -38.0, -63.0, coarse=True, as_pset=True)
    ...
    print(f'Eulerian simulation with starting month {month} is finished')
    ##################################################################################################
