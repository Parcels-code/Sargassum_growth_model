...
def di_Stokes_drift(particles, fieldset):
    ...
    in_wave = T_p > 1E-14 #& (particles.z < local_bathymetry)]

    #Compute particle displacement based on depth-integrated Stokes velocity
    ptcls_wave = particles[in_wave]
    ptcls_wave.dlon += stokes_U_integrated[in_wave] * ptcls_wave.dt
    ptcls_wave.dlat += stokes_V_integrated[in_wave] * ptcls_wave.dt
